...
def find_shortest_path(tubemap, start, end):
    if start == end:
        return [start]
    ...
    while queue:
        ...
        for neighbor in tubemap.get(station, []):
...
def main():
    ...
    if len(parts) != 2:
        print("Invalid input format. Use: StationA,StationB")
        return
    ...
    route = find_shortest_path(tubemap, start, end)
    if route:
